document.addEventListener('DOMContentLoaded', () => {
    const categoriaLink = document.getElementById('series-link');
    ...
    categoriaLink.addEventListener('click', async (e) => {
        ...
        try {
            const response = await fetch('http://localhost:3008/categoria/serie');
    ...
    function generarCartas(data) {
        ...
        container.innerHTML = `
            <h2 class="text-white mb-4 text-center">Catálogo de Series</h2>
    ...
    function abrirModal(carta) {
        ...
        let modal = document.getElementById('modal-series');
        if (!modal) {
            modal = document.createElement('div');
            modal.id = 'modal-series';
